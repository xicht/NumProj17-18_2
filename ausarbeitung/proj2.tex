...
\section{Aufgabe a}
Das Ziel dieser Aufgabenstellung ist es den Graphen, der durch die Nullstellenmenge einer Funktion $\R^2\to\R$ beschrieben ist, numerisch anzunähern. Im ersten Schritt wollen wir dazu einem Gitter $x_j = x_0 + j\cdot h, j \in \left\lbrace1...n\right\rbrace $ entlang $x$ folgen. 
...
Die einfachste Variante auf Aufgabe a basierend Aufgabe b zu implementieren ist rekursiv den Algorithmus aufzurufen, wenn die betragsmäßige Ableitung in eine Richtung größer wird als die andere. Wichtig dabei ist darauf zu achten, dass der Wechsel der Richtung früh genug erfolgt, damit der Bereich mit Nullstellen nicht in eine Richtung verlassen wird, und dass der Richtungswechsel mit dem korrekten Vorzeichen passiert. \\
\begin{algorithm}[H]
	\label{alg:B_V1}
	\SetKwInOut{Input}{Input}
	\SetKwInOut{Output}{Output}
	...
	\caption{Kurve B1}
\end{algorithm}
Nachteile dieser Implementierung liegen darin, dass es aufwändig ist vorherzusagen, wie lange die Kurve wird. In Algorithmus \ref{alg:A_const_h} läuft die Kurve von $\left\lbrace x_0\right\rbrace \times\mathbb{R}$ bis $\left\lbrace x_0+\cdot n\right\rbrace \times\mathbb{R}$. Das bedeutet für $m\in \mathbb{N}$, dass man einen Viertelkreis mit Radius 1 mit $n=m$ und $h=\frac{1}{m}$ zeichnen kann. Mit Algorithmus \ref{alg:B_V1} benötigt man dafür $n=\lceil m*\sqrt{2}\rceil$ und $h=\frac{1}{m}$, da der Richtungswechsel bei $x_i\approx \sin(\frac{\pi}{4})$ stattfindet.
...
\[
\left| \diff{G_\phi}{y}(x_0,y_0) \right| =  \sqrt{\norm{DF(x_0,y_0)}{2}^2-\left(\diff{G_\phi}{x}(x_0,y_0)\right)^2}.
\]
...
\begin{algorithm}[h]
	...
	\SetKwInOut{Output}{Output}
	\Input{$F$...Funktion\\
		$(x,y)_n$...bisher berechnete Punkte\\
		$h$...Schrittweite\\
		$i$...Schrittzahl aktuell}
	\Output{$(x_i,y_i)$}
	$(a,b) = (x_{i-1},y_{i-1})$\\
...
\begin{figure}[H]
	...
	\begin{minipage}{.5\textwidth}
		...
		\captionof{figure}{$(x^2+y^2)^2-x^2+y^2=0$\protect\\$F=\diff{F}{x}=\diff{F}{y}=0$ in $(0,0)$}
		\label{fig:test1}
	\end{minipage}%
	\begin{minipage}{.5\textwidth}
		\centering
		\includegraphics[width=1\linewidth]{plots/C/Herz.eps}
		\captionof{figure}{$(x^2+y^2-1)^3-x^2y^3=0$\protect\\$F=\diff{F}{x}=\diff{F}{y}=0$ in $(0,\pm1)$}
		\label{fig:test2}
	\end{minipage}
\end{figure}

\section{Implementierung von adaptiver Schrittweite}\label{chap:Schrittweite}
Das Ziel von adaptiver Schrittweite ist im Idealfall, dass der Abstand des Polygonzugs durch die Punkte $(x_i,y_i)$ zur Nullstellenmenge kleiner ist als eine vorgegebene Konstante und dafür möglichst wenige Punkte bzw. möglichst wenig Rechenzeit benötigt werden.

Für die adaptive Schrittweite ergeben sich zwei Problemstellungen:
\begin{enumerate}
...
Die zweite Problemstellung ist die wesentlich interessanter, die im folgenden zu lösen versucht wird. Der Einfachheit halber werden mögliche Strategien für adaptive Schrittweite zuerst an der Implementierung aus Aufgabe a getestet, da das Koordinatensystem dort noch fest ist.

Wir gehen bei den Überlegungen dazu daher davon aus, dass die Datenpunkte selbst korrekt sind bzw. dass wir deren Fehler vernachlässigen können.
...
\subsubsection{Abschätzung der Schrittweite anhand der Krümmung von $f$}\label{chap:5.1.1}
 ...
 Zusätzlich dazu nehmen wir an, dass $F \in C^2((a,b))$, was nach Satz 2 eine hinreichende Bedingung für $f \in C^2((a,b))$ ist. Außerdem liefert der Satz dann eine explizite Darstellung von $f''$.  
...
Bei einem einzelnen Iterationsschritt handelt es sich dabei um eine lineare Interpolationsaufgabe, da wir nach dem Korrektorschritt davon ausgehen können, dass $f(x_0)$ und $f(x_0+h)$ nahezu korrekt sind. Es geht hier also um die Frage, wie sich $f$ zwischen diesen Punkten verhält, insbesondere wie sehr es von einer Geraden durch die Punkte $(x_0,f(x_0))$ und $(x_0+h,f(x_0+h))$ abweicht.
...
Ein Nachteil ist, dass man erst nach dem Prediktor- und Korrektschritt $d$ erhält und dann "falls $d$ zu groß war", Prediktor- und Korrektschritt wiederholen muss. Das zweite Problem, ist dass vorerst nicht klar, ist ab welcher Schranke für $d$ man den Iterationsschritt wiederholen soll und um wie viel keiner die Schrittweite dann sein soll.
...
\]
Wegen $|f-t_i|(x_{i+1})=d$ kann man daraus folgern, dass
\[
...
\begin{itemize}
	\item Die ersten sechs sind $F$ bzw dessen Ableitungen (werden die Ableitungen 2. Ordnung nicht gebraucht, entfallen sie).
...
Man sieht in Abbildung \ref{fig:fehler1}, dass die Fehler bei allen Methoden mit der Größenordnung $\mathcal{O}(n^{-2})$ gegen Null gehen. Für äquidistante Stützstellen ist die Schrittweite $h$ proportional zu $n^{-1}$ ist, ist das Ergebnis in dem Fall nicht überraschend. Das entspricht nämlich genau der aus der Vorlesung bekannten (siehe Numerik-Skriptum Bsp 4.9) Konvergenzaussage der summierten Trapezregel: Der Fehler des Integrals geht mit $\mathcal{O}(h^2)$ gegen null.

Die Methoden mit adaptiver Schrittweite haben also alle die selbe Konvergenzggeschwindigkeit, sind allerdings alle um einen Faktor in der Größenordnung 10 besser. Zwischen den einzelnen Strategien ist kein sehr großer Unterschied, wobei die gemischte Strategie etwas besser ist gefolgt von der Betrachtung der Länge des Korrektorschrittes. Daraus dass der Faktor zwischen den Fehlern mit adaptiver Schrittweite und dem ohne bei wachsendem $n$ nicht kleiner wird, kann man schließen, dass die apadaptive Schrittweite auch für kleine Schrittweiten noch gut funktioniert und sinnvoll ist.
...
Da die Anzahl der Datenpunkte für unterschiedliche Niveaulinien sehr unterschiedlich sein kann (zB wegen unterschiedlicher Länge oder Krümmung) und man die jeweiligen Anzahl je nach Algorithmus im Vorhinein auch nicht weiß, ist es nicht sinnvoll, alle Vektoren der $x$- bzw. $y$-Werte der Punkte für die einzelnen Niveau-Linien in gemeinsame Matrizen der Art $X, Y \in \R^{k \times maxSteps}$ zu schreiben. Stattdessen bietet sich ein \verb|cell|-Arays an, der $k$ Vektoren der Länge \verb|Steps(j)| enthält. Der Zugriff auf die einzelnen Vektoren erfolgt durch \verb|X{j}|.
...
\begin{algorithm}[H]
	...
	\If{abs(F(m))<TOL}{ $(x_0,y_0)=(m_x,m_y)$ \\ return
	}
...
Der gesamte Code und diese Ausarbeitung sind unter \url{https://github.com/xicht/NumProj17-18_2} auffindbar.
